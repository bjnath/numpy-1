...
NumPy is a Python package for efficiently performing operations, usually
numeric, on tens or millions of data points.

It marries the usability of Python to the speed of languages like C.

In addition, it's become a world language for handling scientific data.
Packages that have followed NumPy have also followed NumPy's way of doing
things. NumPy knowledge is a passport to more specialized packages that also
operate on arrays.
...
Some aspects of NumPy you barely need a manual for; it's easy to guess that
the result of adding one-dimensional arrays,
::

    c = a + b


is an array where each cell of ``c`` contains  ``a[i] + b[i]``, or that the result of
::

    b = sin(a)

is an array where each cell of ``b`` contains ``sin(a[i])``.


But then come surprises. The result of

::

    a == b

is not ``True`` or ``False``, but an array of booleans, with each cell giving
the result of ``a[i] == b[i]`` for that index.

This illustrates a fundamental approach in NumPy: **operations
are performed, and results supplied, element-by-element.**

So the result of multiplying two arrays
::

    c = a * b

is -- just as if it had been addition -- an array in which cell of ``c`` is the
product of the corresponding cells of ``a`` and ``b``. (Ordinary matrix multiplication
is still available, using the operator ``@``).

NumPy also provides a do-what-I-mean convenience called broadcasting, which
...
- To learn more about the machinery behind NumPy, visit XXX.

- To glimpse *everything* NumPy offers, see the Routines page.
...
so on. NumPy gives two dozen types to choose from. Try it out. Learn more.
...
Dimensions are numbered from the left, starting from 0.

The number of tuple elements is the number of dimensions.
...
Array size is fixed. Once an array is created, the number of elements cannot
change. But:

* You can redistribute which axes the elements lie on (the "shape"), so
  that the length of one dimension can grow as another shrinks. Learn more.

* You can append arrays to make a new larger array. Learn more.

* An array can be split into two or more smaller arrays. Learn more.
...
* You can sometimes make an array behave as though it were larger, matching it
  ...
  processing. This is broadcasting. Learn more.
